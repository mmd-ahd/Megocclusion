...
decoding_dir = os.path.join(derivatives_dir, 'decoding_vertex_svm_rbf_0.1')
    ...
    for level in occlusion_levels:
        ...
        for roi in merged_labels.keys():
            ...
            for cat_1, cat_2 in category_pairs:
                ...
                n_epochs_1 = epochs_1.__len__()
                n_epochs_2 = epochs_2.__len__()
                
                y = np.concatenate((np.zeros(n_epochs_1), np.ones(n_epochs_2)))
                ...
                win_size = int(win_len * 250) 
                step_size = int(win_step * 250)
                ...
                pipeline = make_pipeline(
                    Vectorizer(),
                    StandardScaler(),
                    SVC(kernel='rbf', C=0.1, random_state=715)
                )
                ...
                npy_fname = os.path.join(sub_pair_dir, f'scores_{level}_{cat_1}_{cat_2}_{roi}.npy')
                np.save(npy_fname, scores_mean)
                
                all_pair_scores.append(scores_mean)
            
            if all_pair_scores:
                avg_scores = np.mean(all_pair_scores, axis=0)
                
                fig, ax = plt.subplots(1)
                ax.plot(time_centers, avg_scores, label='score')
                ax.axhline(0.5, color='k', linestyle='--', label='chance')
                ax.set_xlabel('Time (s)')
                ax.set_ylabel('Accuracy')
                ax.legend()
                ax.set_title(f'Decoding in {roi} (Averaged)')
                plt.tight_layout()
                
                fig_fname = os.path.join(sub_level_dir, f'scores_{level}_{roi}_avg.png')
