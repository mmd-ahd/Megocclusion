...
for level_a, level_b in CONTRASTS:
    ...
    for sub_id in SUB_IDS:
        ...
        base_fname_a = os.path.join(stc_dir, sub_bids_id, 'meg', f'{sub_bids_id}_task-objectrecognition_occlusion-{level_a}_meg')
        base_fname_b = os.path.join(stc_dir, sub_bids_id, 'meg', f'{sub_bids_id}_task-objectrecognition_occlusion-{level_b}_meg')
        try:
    ...
    for tmin, tmax in TIME_WINDOWS:
        ...
        stcs_a = [stc.copy().crop(tmin=tmin, tmax=tmax) for stc in stcs_a_full]
        stcs_b = [stc.copy().crop(tmin=tmin, tmax=tmax) for stc in stcs_b_full]

        X_diff = np.array([a.data - b.data for a, b in zip(stcs_a, stcs_b)])
        X_diff = np.transpose(X_diff, (0, 2, 1))

        t_threshold = -stats.distributions.t.ppf(P_THRESHOLD / 2., len(SUB_IDS) - 1)
        ...
        np.savez(
            ...
            H0=H0,
            tmin=tmin,
            tstep=stcs_a[0].tstep
        )
        
        n_significant = np.sum(cluster_pv < 0.05)
